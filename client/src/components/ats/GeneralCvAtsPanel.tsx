    ...
    return (
        <div className="space-y-6">
            ...
            {atsScores.score !== null && atsScores.score !== undefined && (
                <div className="mb-6 p-8 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-blue-900/20 dark:via-indigo-900/20 dark:to-purple-900/20 border-2 border-blue-200 dark:border-blue-800 rounded-2xl shadow-lg">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="flex-1">
                            <h3 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">Overall ATS Compatibility</h3>
                            <p className="text-sm text-gray-600 dark:text-gray-400">Your CV's compatibility with Applicant Tracking Systems</p>
                        </div>
                            ...
                            <div className="relative w-32 h-32">
                                <svg className="transform -rotate-90 w-32 h-32">
                                    ...
                                    <circle
                                        ...
                                        strokeDashoffset={`${2 * Math.PI * 56 * (1 - atsScores.score / 100)}`}
                                        className={`transition-all duration-1000 ${atsScores.score >= 80 ? 'text-green-500 dark:text-green-400' :
                                                atsScores.score >= 60 ? 'text-yellow-500 dark:text-yellow-400' :
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <span className={`text-4xl font-bold ${atsScores.score >= 80 ? 'text-green-600 dark:text-green-400' :
                                            atsScores.score >= 60 ? 'text-yellow-600 dark:text-yellow-400' :
                                                'text-red-600 dark:text-red-400'
                                        }`}>
                                        {Math.round(atsScores.score)}
                                    </span>
                                    <span className="text-xl font-semibold text-gray-500 dark:text-gray-400 ml-1">%</span>
                                </div>
                            </div>
                            {/* Score Label */}
                            <div className="text-center">
                                <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full font-semibold text-sm ${atsScores.score >= 80 ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' :
                                        atsScores.score >= 60 ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300' :
