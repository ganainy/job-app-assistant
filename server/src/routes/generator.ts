...
const generateDocumentsHandler: RequestHandler = async (req, res): Promise<void> => {
    ...
    const requestedLanguage = req.body.language === 'de' ? 'de' : 'en'; // Simple validation for now
    const languageName = requestedLanguage === 'de' ? 'German' : 'English'; // For prompt clarity
    ...
    try {
        ...
        const prompt = `
            You are an expert career advisor and document writer. Your task is to tailor a base CV and write a compelling cover letter for a specific job application IN THE SPECIFIED LANGUAGE.
            ...
            **Instructions:**
            ...
            A.  **Tailor the CV (in ${languageName}):**
                *   Analyze the Base CV Data and the Target Job Description.
                *   Identify the most relevant skills, experiences, and qualifications...
                *   Rewrite or rephrase bullet points... to *emphasize* these relevant aspects IN ${languageName}, using keywords from the Job Description where appropriate and natural.
                *   Ensure the core facts... remain unchanged (translate names/locations only if absolutely necessary and conventional).
                *   Optimize the order of skills or bullet points...
                *   The output for the tailored CV must be a valid JSON object following the exact same structure and keys as the input Base CV Data (personalInfo, summary, experience, education, skills), with ALL text content translated into ${languageName}.

            B.  **Write the Cover Letter (in ${languageName}):**
                *   Generate a concise and professional cover letter (approx. 3-4 paragraphs) IN ${languageName}.
                *   Address it appropriately (e.g., "Dear Hiring Manager," or German equivalent "Sehr geehrte Damen und Herren,").
                *   Briefly introduce the applicant and the specific role... (use jobTitle: "${job.jobTitle}", companyName: "${job.companyName}").
                *   Highlight 2-3 key qualifications... matching the Job Description. Provide brief examples.
                *   Express enthusiasm for the role and the company.
                *   Conclude professionally...
                *   The output for the cover letter must be a single string of text IN ${languageName}.
            ...
            **Output Format:**
            ...
            1.  \`tailoredCvJson\`: The complete, tailored CV data as a JSON object (in ${languageName}).
            2.  \`coverLetterText\`: The complete cover letter as a single string (in ${languageName}).
        `;
        ...
        // Add language code to filenames
        const cvFilenamePrefix = `CV_${req.user.email.split('@')[0]}_${safeCompanyName}_${safeJobTitle}_${requestedLanguage}`;
        const clFilenamePrefix = `CoverLetter_${req.user.email.split('@')[0]}_${safeCompanyName}_${safeJobTitle}_${requestedLanguage}`;
